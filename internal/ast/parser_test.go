...
`)
	...
	pf, err := p.ParseFile(t.Context(), path)
	if err != nil {
		t.Fatal(err)
	}
...
`)
	...
	pf1, err := p.ParseFile(t.Context(), path)
	if err != nil {
		t.Fatal(err)
	}
	pf2, err := p.ParseFile(t.Context(), path)
	if err != nil {
		t.Fatal(err)
	}
	if pf1 != pf2 {
...
`)
	...
	files, err := p.ParseDir(t.Context(), dir)
	if err != nil {
		t.Fatal(err)
	}
...
`)
	...
	files := p.ParseFiles(t.Context(), dir, []string{"a.go", "b.go"})
	if len(files) != 2 {
