...
type General struct {
	...
	TaskQueue              string                 `toml:"task_queue"`               // Temporal task queue name (default chum-task-queue)
}
...
type DoDConfig struct {
	...
	Web               WebDoDConfig `toml:"web"`                // web verification config (Stingray Web)
}

// WebDoDConfig configures web-based DoD verification (Stingray Web).
// When enabled, deployed URLs are verified with HTTP smoke tests,
