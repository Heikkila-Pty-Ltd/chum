...
// Selection is the selected branch returned by SelectUCT.
...
// SelectUCT picks the highest-scoring branch using UCT.
func SelectUCT(arms []Arm, exploration float64) (Selection, bool) {
	if len(arms) == 0 {
	...
	bestKey := ""
	for i := range arms {
		score := uctScore(arms[i], totalVisits, exploration)
		prior := arms[i].Prior
		key := arms[i].Key
		if score > best.Score ||
...
func uctScore(arm Arm, totalVisits int, exploration float64) float64 {
	visits := arm.Visits
	if visits <= 0 {
		return math.Inf(1)
	}
