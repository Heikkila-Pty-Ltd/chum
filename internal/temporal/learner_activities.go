...
// CommitLearnerOutputsActivity commits the CLAUDE.md, .semgrep rules,
// and .cortex/calcified scripts to the local worktree repository.
func (a *Activities) CommitLearnerOutputsActivity(ctx context.Context, workDir string, taskID string) (bool, error) {
	logger := activity.GetLogger(ctx)
	logger.Info(OctopusPrefix+" Committing learner outputs to worktree", "WorkDir", workDir)
	...
	logger.Info(OctopusPrefix + " Learner outputs committed successfully")
	return true, nil
}
